...
LangGraph 版「自主式 SOC Tier-1 事件回應代理」的**基礎骨架（計畫 0）已完成並合併到 `main`**，**計畫 A（告警分流）、C（研判與劇本）、D（編排與安全）三個整合邊界都已完成並合併，且計畫 D 的 Streamlit Demo UI 也已完成**，112 個測試全綠、CLI 可跑、Demo 可互動展示。唯一剩下的子系統是計畫 B（組員 2 進行中）；其餘是 A/C 的 LLM／訓練軌（邊界已就緒，下游零改動）。
...
**git 狀態**：在 `main`（原 `master` 已改名），最新是 `416c3e1`（Demo UI merge + uv.lock 同步）。功能分支用完即刪。**已推上 GitHub（Private）**：`origin` → https://github.com/JoeW00/llm_security_term_project.git，`main` 追蹤 `origin/main`。注意推送需用 `JoeW00` gh 帳號（`gh auth switch --user JoeW00`）。
...
tests/              # 112 個測試（骨架 + A：classifier/triage_injection/ingest_iocs/ollama/triage_eval + C：reasoning/investigator/playbook_reasoner/critic/reasoning_nodes/anthropic_client/reasoning_eval + D：approval/reporting/approval_nodes/injection_eval/demo_controller）
```
...
**A/C/D 整合邊界 + 計畫 D Demo UI 皆已完成。剩下的工作都是「接真實後端／資料／訓練」——邊界已就緒、下游零改動：**
...
4. **計畫 D live 數字**：把上面接好的 live 圖當 runner，跑 `demo/controller.injection_report` / `eval/injection_eval.py` 出真實「防禦前後」注入操控率 + 用 `eval/runtime_metrics.end_to_end_metrics` 出延遲／迭代數（目前確定性基線 manipulation rate = 0%）。Demo UI 可在 live 接好後直接展示真實推理（`IncidentSession` 的 `build` 換成注入 LLM 推理器的版本）。
...
- **計畫 D Demo UI（已完成）+ live 注入軌注意**：互動 Demo 已在 `demo/`（`uv run --group demo streamlit run demo/app.py`）；編排邏輯在不依賴 streamlit 的 `demo/controller.py`（`IncidentSession`），UI 只是薄層。接 live 時：(1) 把 `IncidentSession.__init__` 的 `build` 換成「注入 LLM 推理器版本」即可讓 Demo 展示真實推理；(2) UI 的人工核准送 `Command(resume={"approved": bool, "reason": str})`——`approved` 必須是**真 bool**（`InterruptApprovalPolicy` 用 strict 驗證，`"true"`/`1` 會被當畸形而保守駁回）；(3) `InterruptApprovalPolicy.decide` 在 `interrupt()` 前無副作用、可安全 replay，**未來若把真實處置動作放進 human_approval，務必擺在 `interrupt()` 之後**（resume 會重跑該節點）；(4) 真實注入「防禦前後」數字：把 `demo/controller.injection_report` 的 `build` 換成 live 版本。
