...
同時完整展示 LangGraph 的狀態管理、條件路由與迴圈價值。流程圖見
`docs/superpowers/specs/assets/soc-agent-flow.png`。
...
*圖 4.3：情資增強信任邊界的端到端實證（`_evidence/demo_enrich.py`）。即時惡意樣本為當日 ThreatFox 新增、內容隨時間變動；私有 IP 與中性降級兩路徑為確定性行為。*
...
**E2（A）　分流四臂消融 + 類別平衡重訓**（DGX Spark 實機存檔，`results/W15-P1-C/B2`）：
...
> 截圖原檔位於 `docs/reports/assets/W16-evidence-E*.png`，由 `_evidence/render.py` 從各次
> 實際執行輸出渲染；A 軸（四臂消融）因需 2.4 GB GUIDE 資料與 32B 模型，為 DGX Spark 實機之
> 存檔結果（`results/W15-P1-*`），其餘各項可於本機重現。Demo UI 操作截圖
> （`assets/W16-demo-D*.png`）為 Streamlit 實機畫面，見 §3.4 圖 3.5–3.7。
...
# 可選的雲端 / 金鑰路徑：
ANTHROPIC_API_KEY=... uv run --group llm python -m soc_agent run <alert> --llm   # C 雲端研判
ABUSE_CH_AUTH_KEY=...  uv run --group intel ...          # B live 情資（abuse.ch）
```
...
| 九節點 / 圖 / 路由 | `soc_agent/nodes.py`、`graph.py`、`routing.py` |
| A 分流 | `soc_agent/classifier.py`、`classifiers/` |
| B 情資 + ATT&CK | `soc_agent/enrichment.py`、`enrichers/`、`attack.py`、`attack_mappers/` |
| C 研判 + 劇本 | `soc_agent/reasoning.py`、`reasoners/` |
| D 核准 + 報告 | `soc_agent/approval.py`、`reporting.py` |
| 評估框架 | `eval/`、`scripts/eval/` |
| 真實量測 | `results/W15-*`、`results/W16-*` |
| 實驗證據截圖 | `docs/reports/assets/W16-evidence-*.png`（§9）、`W16-demo-D*.png`（§3.4 Demo UI） |
| 設計文件 | `docs/superpowers/specs/`、`plans/` |
